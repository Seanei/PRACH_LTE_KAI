...
class PreambleGeneratorBlock(Block):
    ...
    def process(self, data: CommonData) -> CommonData:
        ...
        u_zc = U_ZC_FDD[self.root_sequence_index]  # logical root to physical

        c_v = 0

        if self.high_speed_flag == 0:
            if n_cs == 0:
                c_v = 0
            else:
                v = self._random_system.randrange(math.floor(n_zc / n_cs))
                c_v = v * n_cs
        else:
            while self.root_sequnce_index < n_zc:
                p = 0
                while (p * u_zc) % n_zc != 1:
                    p += 1

                d_u = p if (0 <= p and p < n_zc / 2) else n_zc - p
                ...
                elif n_zc / 3 <= d_u and d_u <= (n_zc - n_cs) / 2:
                    n_shift = math.floor((n_zc - 2 * d_u) / n_cs)
                    d_start = n_zc - 2 * d_u + n_shift * n_cs
                    n_group = math.floor(d_u / d_start)
                    n_shift_avg = min(
                        max(math.floor((d_u - n_group * d_start) / n_cs), 0), n_shift
                    )
                else:
                    # Additional preamble sequences, in case 64 preambles cannot be
                    # generated from a single root Zadoff-Chu sequence, are obtained
                    # from the root sequences with the consecutive logical indexes until
                    # all the 64 sequences are found

                    self.root_sequnce_index += 1
                    u_zc = U_ZC_FDD[self.root_sequence_index]

                    continue

            v = self._random_system.randrange(n_shift * n_group + n_shift_avg - 1)
            c_v = d_start * math.floor(v / n_shift) + (v % n_shift) * n_cs

        zadoff_chu_seq = zadoff_chu(n_zc, u_zc)
