...
N_ZC_FDD = 839  # formats 0-3
N_ZC_TDD = 139  # format 4

# highSpeedFlag used to determine if cell is used
# for low or high mobility UEs
# Used to cope with large Doppler spread

# Ncs from zeroCorrelationZoneConfig and highSpeedFlag
# zeroCorrelationZoneConfig is in range 0..=15 and highSpeedFlag in range 0..=1
# Usage example: array[zeroCorrealationZoneConfig][highSpeedFlag]
...
class PreambleGeneratorBlock(Block):
    ...
    def _generate_preambles(
        ...
        while len(preambles) < total_preambles:
            ...
            shifts = self._get_shifts(n_zc, n_cs, u_zc * high_speed_flag)  # hs = 0 or 1
    ...
    def _get_shifts(self, n_zc: int, n_cs: int, u_zc: int = 0) -> list[int]:
        return (
            self._get_shifts_unrestricted(n_zc, n_cs)
            if u_zc == 0
            else self._get_shifts_restricted(n_zc, n_cs, u_zc)
        )

    # TODO: cache it?
    def _get_shifts_unrestricted(self, n_zc: int, n_cs: int) -> list[int]:
        if n_cs == 0:
            return [0]
    ...
    # TODO: cache it?
    def _get_shifts_restricted(self, n_zc: int, n_cs: int, u_zc: int) -> list[int]:
        if n_cs == 0:
            return [0]
        ...
        if d_u >= n_zc / 3:
            ...
            n_shift_avg = max(
                math.floor((n_zc - 2 * d_u - n_group * d_start) / n_cs), 0
            )
        else:
    ...
    def process(self, data: CommonData) -> CommonData:
        # TODO: add 2 another functions to override for `Block`
        #       to extract configuration variables from `CommonData`
        #       and to validate it before processing

        # get higher levels configuration
        # use local configuration instead if none is present
        self.root_sequence_index = data.meta.get(
            "root_sequence_index", self.root_sequence_index
        )
        self.preamble_format = data.meta.get("preamble_format", self.preamble_format)
        self.preamble_index = data.meta.get("preamble_index", self.preamble_index)
        ...
        all_preambles = self._generate_preambles(
            self.root_sequence_index,
            self.zero_correlation_config,
            self.high_speed_flag,
            N_ZC_FDD,
            U_ZC_FDD,
            N_CS_FDD,
        ...
        selected_preamble = all_preambles[preamble_index]

        data.meta["generated_preamble"] = selected_preamble
